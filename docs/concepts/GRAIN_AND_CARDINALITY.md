...
Each source is aggregated independently to this grain and tested for a unique key before joining. The join uses both practice and month. The integrated row count must equal the union-spine key count, which gives a row-multiplication factor of 1.0.
