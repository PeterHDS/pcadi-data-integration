...
The runner returns a nonzero status and writes `BLOCKERS_AND_EXCEPTIONS.md` if a required gate fails. A failed build must not be described or packaged as a validated release. The blocker file is removed only by a later successful complete run.
